...
class DB_CONN {
	...
	public function select_modules($id = NULL){
        ...
        //returns PDO to fetch of existent rows
	public function select_rows($id = NULL){
		if ($id==NULL){
			 $sql = "SELECT * FROM tbRows ORDER BY RowName ASC;";
			 $params = array();
		} else {
			$sql = "SELECT * FROM tbRows WHERE RowID = :id ORDER BY RowName ASC;";
			$params = array(':id'=>$id);
		}
	...
	public function query_module_add($name, $description, $gpio, $throughtput, $image, $row){
		$sql = "INSERT INTO tbModules (Name, Description, GPIO, Throughtput, RowId) ";
		$sql.= "VALUES (:name, :description, :gpio, :throughtput, :row)";
        ...
        public function query_row_add($name, $description, $image){
		$sql = "INSERT INTO tbRows (RowName, RowDescription) ";
        ...
        public function select1_row_exists($id){
		$sql = "SELECT * FROM tbRows WHERE RowID = :id";
